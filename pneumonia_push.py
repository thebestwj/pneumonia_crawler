...
def analyzer(soup):
    ...
    provincials = []
    for provincial in soup.find_all('div', class_='fold___xVOZX'):
        provincials.append(provincial.get_text())
    realtime_news = []
...
def data_processor(mix):
    ...
    for provincial in provincials:
        if re.search('确诊',provincial) is not None:
            prov_determined_count = prov_determined_count + 1
        if re.search('死亡',provincial) is not None:
            prov_death_count = prov_death_count + 1
        if re.search('疑似',provincial) is not None:
    ...
    csv_row = [time_stamp,ts_h,ts_d,ts_m,determined,prob,cured,death,prov_determined_count,prov_prob_count,prov_death_count,zj_determined,mix]
    return csv_row
...
def news_job():
    ...
    # news_titles, links, timestamps = load()
    # print(news_titles, links, timestamps)
    ...
    # # print(len(news_titles))
    #
    # get_news_titles, get_links, get_timestamps = get_news_list()
    # for t in get_news_titles:
    #     if t not in news_titles:
    #         print('new! : ' + str(t))
    #         push_index = get_news_titles.index(t)
    #         push_url = get_links[push_index]
    #         push_context = get_news(push_url)
    #         push_timestamp = get_timestamps[push_index]
    #         # 记录新通知
    #         news_titles.append(t)
    #         links.append(push_url)
    #         timestamps.append(push_timestamp)
    #         # 发送邮件
    #         send_email(t, push_url, push_timestamp, push_context)
    #     else:
    #         print('old! : ' + str(t))
    #
    # save(news_titles, links, timestamps)
